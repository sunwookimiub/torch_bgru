...
from utils import signBNN, signSigmoidBNN
try:
    import torch_bgru.backends.cudnn.rnn
except ImportError:
    pass
...
def GRUCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
    ...
    """
    # June 21, 2018. Commented out due to unaccessible GRUCell code.
    # Hence, use non cuda code below this segment. 
...
def Binary_GRUCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None):
    ...
    binary_sigmoid = signSigmoidBNN()
    binary_tanh = signBNN()
    resetgate = binary_sigmoid(i_r + h_r)
    inputgate = binary_sigmoid(i_i + h_i)
    newgate = binary_tanh(i_n + resetgate * h_n)
    hy = newgate + inputgate * (hidden - newgate)
...
def AutogradRNN(mode, input_size, hidden_size, num_layers=1, batch_first=False,
    ...
    if mode == 'RNN_RELU':
        ...
        cell = LSTMCell
    elif mode == 'GRU' or mode == 'GRU_AutogradRNN':
        cell = GRUCell
    elif mode == 'Binary_GRU':
        cell = Binary_GRUCell
    else:
...
def RNN(*args, **kwargs):
    ...
    mode = (*args,)[0]
    # print ('_functions RNN called with mode: {}'.format(mode))

    def forward(input, *fargs, **fkwargs):
        if mode != 'GRU_AutogradRNN' and mode != 'Binary_GRU' and cudnn.is_acceptable(input.data):
            #print ("cudnn used")
